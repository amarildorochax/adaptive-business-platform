...
export class OfficeScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet(
      "anim-door",
      "/sprites/animated_door_big_4_48x48.png",
      {
        frameWidth: 48,
        frameHeight: 144,
      }
...
============================`);
    ...
    this.cleanups.push(
      ...
      eventBridge.on(
        "handoff",
        (from, to) => this.animateHandoff(from, to)
      )
    );
...
CADEIRAS: ${this.seats.length}
==========================`
    );

    const sorted = [...agents].sort((a, b) => {
      if (a.desk.row !== b.desk.row) {
        return a.desk.row - b.desk.row;
      }
