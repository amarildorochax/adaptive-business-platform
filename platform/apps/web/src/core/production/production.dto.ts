/**
 * DTOs da camada Frontend do Production Hub — cópia deliberada de `apps/api/src/dtos/production.dto.ts`
 * (IMP-503), nunca um import cross-app. Nenhum tipo de `@abp/production-hub` é importado aqui, mesma
 * disciplina de `inventoryMovement.dto.ts`/`purchase.dto.ts`/`supplier.dto.ts` (Frontend).
 *
 * Nenhum DTO de atualização parcial (`PATCH`) existe neste arquivo — mesma auditoria já realizada por
 * IMP-503: nenhum dos nove Commands do Production Hub corresponde a uma atualização parcial por merge.
 */
