...
/**
 * Cópia deliberada de `core/purchase/testing/realApiServer.ts`/`core/supplier/testing/realApiServer.ts`
 * (IMP-304/IMP-204) — o arquivo é inteiramente genérico (sobe todo `apps/api`, não apenas rotas do
 * Inventory Movement Hub) e por isso permanece um candidato real a um helper compartilhado
 * (`core/http/testing/realApiServer.ts`), identificado desde `IMP_304_PURCHASE_FRONTEND_REPORT.md` e
 * reafirmado aqui — ainda não extraído nesta Sprint, per instrução explícita ("Não refatorar neste
 * Sprint"), seguindo à risca o padrão já consolidado pela terceira vez consecutiva.
 *
 ...
 * IMP-204/IMP-304. Corrigido apenas neste helper de teste: `globalThis.URL` é temporariamente
 ...
 * integração real do Inventory Movement Hub — nunca por nenhum componente de produção.
 */
export async function startRealApiServer(beforeListen?: (fastify: FastifyInstance) => void): Promise<RealApiServer> {
  const fastify = await importWithNodeUrl(async () => {
    ...
    // mesma decisão documentada em IMP-204/IMP-304: `@abp/api/package.json` não declara `exports`, e
