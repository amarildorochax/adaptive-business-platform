...
/**
 * Cópia deliberada de `core/production/testing/realApiServer.ts`/`core/inventory-movement/testing/realApiServer.ts`/
 * `core/purchase/testing/realApiServer.ts`/`core/supplier/testing/realApiServer.ts` (IMP-204/304/404/504)
 * — o arquivo é inteiramente genérico (sobe todo `apps/api`, não apenas rotas do Fiscal Hub) e por isso
 * permanece um candidato real a um helper compartilhado (`core/http/testing/realApiServer.ts`),
 * identificado desde `IMP_304_PURCHASE_FRONTEND_REPORT.md` e reafirmado a cada Sprint subsequente —
 * ainda não extraído nesta Sprint também, per instrução explícita ("Não refatorar neste Sprint"),
 * seguindo à risca o padrão já consolidado pela quinta vez consecutiva.
 *
 ...
 * IMP-204/IMP-304/IMP-404/IMP-504. Corrigido apenas neste helper de teste: `globalThis.URL` é
 * temporariamente restaurado à classe nativa do Node antes de importar (dinamicamente)
 ...
 * integração real do Fiscal Hub — nunca por nenhum componente de produção.
 */
export async function startRealApiServer(beforeListen?: (fastify: FastifyInstance) => void): Promise<RealApiServer> {
  const fastify = await importWithNodeUrl(async () => {
    ...
    // mesma decisão documentada em IMP-204/IMP-304/IMP-404/IMP-504: `@abp/api/package.json` não
    // declara `exports`, e adicionar um agora contaria como alteração a `apps/api`, proibida nesta
    // Sprint.
