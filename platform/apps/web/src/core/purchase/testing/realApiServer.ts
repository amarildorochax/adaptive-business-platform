...
/**
 * Cópia deliberada de `core/supplier/testing/realApiServer.ts` (IMP-204) — o arquivo é inteiramente
 * genérico (sobe todo `apps/api`, não apenas rotas do Purchase Hub) e por isso é um candidato real a
 * um helper compartilhado (`core/http/testing/realApiServer.ts`), identificado no Capítulo
 * "Qualidade" de `IMP_304_PURCHASE_FRONTEND_REPORT.md` — não extraído nesta Sprint, per instrução
 * explícita ("Não refatorar neste Sprint"), seguindo à risca o padrão já consolidado.
 *
 ...
 * IMP-204. Corrigido apenas neste helper de teste: `globalThis.URL` é temporariamente restaurado à
 * classe nativa do Node antes de importar (dinamicamente) `@abp/persistence` e o `server.ts` de
 * `apps/api` — nunca alterando nenhum dos dois pacotes.
 */
 ...
 * de integração real do Purchase Hub — nunca por nenhum componente de produção.
 */
export async function startRealApiServer(beforeListen?: (fastify: FastifyInstance) => void): Promise<RealApiServer> {
  const fastify = await importWithNodeUrl(async () => {
    ...
    // mesma decisão documentada em IMP-204: `@abp/api/package.json` não declara `exports`, e
    // adicionar um agora contaria como alteração a `apps/api`, proibida nesta Sprint.
